...
@implementation KTPhotoScrollViewController {
    NSMutableDictionary	*tileBuilders;
    ImageScrollView* tiledView;
}
...
- (id)initWithDataSource:(id <KTPhotoBrowserDataSource>)dataSource andStartWithPhotoAtIndex:(NSUInteger)index 
{
   if (self = [super init]) {
     ...
     tileBuilders = [[NSMutableDictionary alloc] init];
...
- (void)loadView 
{
   ...
   if ([dataSource_ respondsToSelector:@selector(exportImageAtIndex:photoView:)])
   {
   ...
   [toolbar_ setBarStyle:UIBarStyleBlackOpaque];
...
- (void)loadPhoto:(NSInteger)index
{
   ...
   if (NO == [currentPhotoView isKindOfClass:[KTPhotoView class]]) {
      ...
      KTPhotoView* photoView = currentPhotoView;
      NSString* key = photoView.imageKey;
       
      TiledImageBuilder* tb = [tileBuilders objectForKey:key];
      if (tb != nil && index == currentIndex_) {
          [self tilePages];
      }
   }
}
...
{
    if ([dataSource_ respondsToSelector:@selector(exportImageAtIndex:photoView:)])
        [dataSource_ exportImageAtIndex:currentIndex_ photoView:[self view]];
...
- (void)tilePages 
{
    if (tiledView == nil || tiledView.tag != currentIndex_) {
        if (tiledView == nil)
            tiledView = [ImageScrollView new];

        // If you want the image to FILL the view, not FIT into the view
        //((ImageScrollView *)page).aspectFill = YES;
        tiledView.tag = currentIndex_;
        tiledView.frame = [self frameForPageAtIndex:currentIndex_];
        KTPhotoView* photoView = [photoViews_ objectAtIndex:currentIndex_];
        NSString* key = photoView.imageKey;
        // Use tiled images
        [tiledView displayObject:[tileBuilders objectForKey:key]];
        [scrollView_ addSubview:tiledView];
    }
}

- (void)addObservers {
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(fullResolutionImageSaved:) name:kFullResSavedNotification object:nil];
}

- (void)removeObservers {
    [[NSNotificationCenter defaultCenter] removeObserver:self name:kFullResSavedNotification object:nil];
    
}

- (void)fullResolutionImageSaved:(NSNotification*)notification {
    NSString* key = [[notification userInfo] objectForKey:@"key"];
    NSString* path = [[notification userInfo] objectForKey:@"path"];
    //NSLog(@"Notification received:\n\tfull res version of image with\n\t\tkey: %@\n\t\tpath: %@", key, path);
    TiledImageBuilder *tb = [[TiledImageBuilder alloc] initWithImagePath:path size:CGSizeMake(320, 320) orientation:0];
    [tileBuilders setObject:tb forKey:key];

    KTPhotoView* currentPhotoView = [photoViews_ objectAtIndex:currentIndex_];

    if ([key isEqualToString:currentPhotoView.imageKey]) {
        [self tilePages];
    }
